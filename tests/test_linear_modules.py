...
@torch.no_grad()
def test_w8a8bfp32ofp32_linear_relu():
    B, M, N = 128, 512, 1024
    x = torch.randn(B, M)
    x_scale = x.abs().max() / 127
    qx = (x / x_scale).round().to(torch.int8)
    linear = torch.nn.Linear(M, N, bias=True)
    y_gt = linear(x).clamp(min=0)
    q_linear = W8A8BFP32OFP32LinearReLU.from_float(linear, x_scale).cuda()
...
if __name__ == '__main__':
    ...
    print('test_w8a8bfp32ofp32_linear_relu')
    test_w8a8bfp32ofp32_linear_relu()
