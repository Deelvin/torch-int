...
@torch.no_grad()
def test_quant_linear_relu_a8_w8_bfp32_ofp32():
    B, M, N = 128, 512, 1024
    weight = torch.randint(-128, 127, (N, M), dtype=torch.int8)
    bias = torch.randn(N, dtype=torch.float32)
    x = torch.randint(-128, 127, (B, M), dtype=torch.int8)
    ...
    linear.bias.data = bias.float() * beta
    y_gt = linear(x.float())
    y = linear_relu_a8_w8_bfp32_ofp32(x.cuda(), weight.cuda(),
                                bias.cuda(), alpha, beta)
    ic(torch.allclose(y_gt, y.cpu(), atol=0.5))
...
if __name__ == '__main__':
    ...
    print('test_quant_linear_relu_a8_w8_bfp32_ofp32')
    test_quant_linear_relu_a8_w8_bfp32_ofp32()
